...
export function A2ACommunication({ messages }: { messages: A2AMessage[] }) {
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {messages.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-1">Start an analysis to see A2A protocol in action</p>
          </div>
        ) : (
          messages.map((message) => {
            ...
            return (
              <div
                key={message.id}
                className={`border rounded-lg p-4 ${messageTypeColors[message.type]}`}
              >
