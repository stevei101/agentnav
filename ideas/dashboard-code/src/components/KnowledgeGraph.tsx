...
export function KnowledgeGraph({ agents }: { agents: AgentStatus[] }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < subConcepts; i++) {
      ...
      const mainAngle = Math.floor(i / (subConcepts / concepts)) * (Math.PI * 2 / concepts);
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-sm text-gray-400">
          {completedAgents > 0 ? `${completedAgents * 52} nodes` : 'Waiting for analysis'}
        </span>
      </div>

      <div className="relative bg-gray-950 rounded-lg overflow-hidden" style={{ height: '400px' }}>
        <canvas
          ref={canvasRef}
