        ...
        try:
            ...
            try:
                response = await generate_content_with_prompt_template(
                    ...
                    template_variables={"content": document[:2000], "entity_count": "5-10"},
                    temperature=0.3,
...
"""
                ...
                response = await generate_content(
                    prompt=prompt, temperature=0.3, max_output_tokens=300
                )
...
"""
                ...
                response = await reason_with_gemma(prompt=prompt, max_tokens=300, temperature=0.3)
    ...
    def _identify_code_relationships(
        ...
        for imp in import_entities[:3]:  # Limit to avoid too many connections
            for func in function_entities[:3]:
                relationships.append(
                    {"from": func["id"], "to": imp["id"], "type": "uses", "label": "imports"}
                )
    ...
    async def _enhance_relationships_with_reasoning(
        self, document: str, entities: List[Dict[str, Any]], relationships: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
...
"""
                ...
                response = await generate_content(
                    prompt=prompt, temperature=0.3, max_output_tokens=400
                )
...
"""
                ...
                response = await reason_with_gemma(prompt=prompt, max_tokens=400, temperature=0.3)
    ...
    def _prepare_graph_data(
        self, entities: List[Dict[str, Any]], relationships: List[Dict[str, Any]], content_type: str
    ) -> Dict[str, Any]:
