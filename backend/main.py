...
# Import Suggestion routes (FR#201)
from backend.routes.suggestion_routes import router as suggestion_router
...
ALLOWED_HOSTS = os.getenv(
    "ALLOWED_HOSTS", "agentnav.lornu.com,*.run.app,localhost,127.0.0.1"
).split(",")
...
# Include Suggestion routes (FR#201 - Prompt Vault Intelligence)
...
async def generate_text(request: GenerateRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Text generation service unavailable: {str(e)}"
        )
...
async def visualize_content(request: VisualizeRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Visualizer Agent not available")
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            diagnostic_info["initialization_errors"].append(
                f"A2A Protocol initialization failed: {str(e)}"
            )
        ...
        logger.info(
            f"✅ ADK status check complete: {response['adk_system']}, {len(agents)}/{len(agent_classes)} agents available"
        )
        return response
