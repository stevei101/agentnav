...
class A2ASecurityContext(BaseModel):
    ...
    service_account_id: Optional[str] = Field(
        None, 
        description="GCP Service Account ID from Cloud Run Workload Identity"
    )
...
class A2ATraceContext(BaseModel):
    ...
    parent_message_id: Optional[str] = Field(
        None,
        description="ID of the parent message in the conversation chain"
    )
    span_id: Optional[str] = Field(
        None,
        description="Distributed tracing span ID (for Cloud Trace integration)"
    )
...
class A2AMessageBase(BaseModel):
    ...
    to_agent: str = Field(
        ...,
        description="Target agent name (use '*' for broadcast)"
    )
    priority: A2AMessagePriority = Field(
        default=A2AMessagePriority.MEDIUM,
        description="Message priority level"
    )
    status: A2AMessageStatus = Field(
        default=A2AMessageStatus.PENDING,
...
class SummarizationCompletedMessage(A2AMessageBase):
    ...
    insights: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional insights from summarization"
    )
...
class VisualizationReadyMessage(A2AMessageBase):
    ...
    # Visualization data
    visualization_type: str = Field(..., description="Type of visualization (MIND_MAP, DEPENDENCY_GRAPH)")
...
class KnowledgeTransferMessage(A2AMessageBase):
    ...
    knowledge_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="Knowledge payload data"
    )
...
class AgentStatusMessage(A2AMessageBase):
    ...
    processing_time_seconds: Optional[float] = Field(
        None,
        description="Time taken for processing (if completed)"
    )
