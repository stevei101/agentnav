...
class AgentStreamEvent(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: f"evt_{uuid.uuid4().hex[:8]}", description="Unique event ID"
    )
    ...
    payload: AgentEventPayload = Field(
        default_factory=AgentEventPayload, description="Event payload with results or error"
    )
