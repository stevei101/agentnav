...
class SessionContext(BaseModel):
    ...
    summary_text: Optional[str] = Field(
        None, description="Comprehensive summary from Summarizer Agent"
    )
    summary_insights: Optional[Dict[str, Any]] = Field(
        None, description="Additional insights from summarization"
    )

    # Linker Agent outputs
    key_entities: List[str] = Field(
        default_factory=list, description="List of key entities identified by Linker Agent"
    )
    ...
    entity_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata about entities"
    )

    # Visualizer Agent outputs
    graph_json: Optional[Dict[str, Any]] = Field(
        None, description="Final visualization-ready graph from Visualizer Agent"
    )

    # Workflow tracking
    completed_agents: List[str] = Field(
        default_factory=list, description="List of agents that have completed processing"
    )
