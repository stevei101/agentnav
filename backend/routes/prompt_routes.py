...
async def get_user_from_header(
    ...
    return UserInfo(
        id=authorization or "default-user", email="user@example.com", name="User"
    )


@router.get("/", response_model=List[Prompt])
async def list_prompts(user: Optional[UserInfo] = Depends(get_user_from_header)):
    """List all prompts"""
    try:
...
async def get_prompt(
    prompt_id: str, user: Optional[UserInfo] = Depends(get_user_from_header)
):
    """Get a prompt by ID"""
    try:
        service = get_prompt_service()
...
async def create_prompt(
    prompt_data: PromptCreate, user: Optional[UserInfo] = Depends(get_user_from_header)
):
    """Create a new prompt"""
    try:
        service = get_prompt_service()
...
async def add_test_result(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add test result: {str(e)}"
        )


@router.get("/user/info", response_model=UserInfo)
async def get_user_info(user: Optional[UserInfo] = Depends(get_user_from_header)):
