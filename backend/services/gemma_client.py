...
def _fetch_id_token(target_url: str) -> Optional[str]:
    ...
    try:
        ...
        response = requests.get(token_url, headers=metadata_flavor, params=params, timeout=5.0)
        ...
        if response.status_code == 200:
            ...
            error_msg = f"Failed to fetch ID token: {response.status_code} {response.text}"
...
class GemmaServiceClient:
    ...
    def __init__(self, base_url: Optional[str] = None):
        self.base_url = base_url or os.getenv(
            "GEMMA_SERVICE_URL", "http://localhost:8080"  # Default for local development
        )
