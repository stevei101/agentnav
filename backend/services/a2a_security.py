...
class ServiceAccountIdentity:
    ...
    def from_cloud_run_metadata(cls) -> "ServiceAccountIdentity":
        ...
        try:
            ...
            email = os.getenv("GCP_SERVICE_ACCOUNT_EMAIL", "unknown@unknown.iam.gserviceaccount.com")
...
class A2ASecurityService:
    ...
    def authenticate_service_account(self, service_account_email: str) -> bool:
        ...
        if not is_trusted:
            ...
            self._log_security_event("untrusted_service_account", {
    ...
    def authorize_agent_communication(self, from_agent: str, to_agent: str, 
        ...
        is_authorized = (
            "*" in allowed_targets or  # Broadcast allowed
            to_agent in allowed_targets or  # Specific target allowed
            to_agent == "*"  # Broadcasting
        )
        
        if not is_authorized:
            logger.warning(f"⚠️  Unauthorized agent communication: {from_agent} → {to_agent}")
            self._log_security_event("unauthorized_communication", {
