...
class EventEmitter:
    ...
    async def emit_agent_processing(
        self, agent: AgentTypeEnum, step: int, partial_results: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    async def emit_agent_complete(
        ...
        event = create_agent_complete_event(
            agent=agent, step=step, elapsed_ms=self._calculate_elapsed_ms(), payload=payload
        )
        await self.emit_event(event)
