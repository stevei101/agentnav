...
async def test_call_service_attaches_bearer_token(monkeypatch):
    ...
    with (
        patch("backend.services.workload_identity_client.httpx.AsyncClient", return_value=mock_context),
        patch("backend.services.workload_identity_client.id_token.fetch_id_token", return_value="token123"),
    ):
        await call_service("https://backend.example.com/api", method="POST", json={"key": "value"})
...
async def test_token_cache(monkeypatch):
    ...
    with (
        patch("backend.services.workload_identity_client.id_token.fetch_id_token", return_value="token123") as fetch,
        patch("backend.services.workload_identity_client.httpx.AsyncClient", return_value=mock_context),
    ):
