...
class TestGeminiClientInitialization:
    ...
    @patch.dict(
        os.environ, {"GOOGLE_CLOUD_PROJECT": "test-project", "GEMINI_API_KEY": ""}
    )
    ...
    @patch.dict(
        os.environ, {"GOOGLE_CLOUD_PROJECT": "", "GEMINI_API_KEY": "test-api-key-12345"}
    )
    @patch("services.gemini_client.genai")
    ...
    @patch.dict(
        os.environ, {"GOOGLE_CLOUD_PROJECT": "", "GEMINI_API_KEY": ""}, clear=True
    )
...
class TestGeminiContentGeneration:
    ...
    async def test_generate_content(self, mock_get_loop, mock_genai):
        ...
        await generate_content(prompt="Test prompt", temperature=0.7)
    ...
    async def test_generate_content_with_prompt_template(
        self, mock_generate, mock_get_prompt
    ):
        ...
        assert (
            "Template with test_value" in call_args[1]["prompt"]
            or "Template with test_value" in call_args[0][0]
        )
