...
def test_gemma_client_optional():
    ...
    gemma_client_path = os.path.join(
        os.path.dirname(__file__), "services", "gemma_client.py"
    )
...
def test_ci_no_gemma_build():
    ...
    try:
        ...
        if (
            "service: [frontend, backend]" in content
            and "gemma"
            not in content.split("service: [frontend, backend]")[1].split("\n")[0]
        ):
