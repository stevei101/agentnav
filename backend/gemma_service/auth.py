...
def verify_jwt_token(authorization: Optional[str] = Header(None)) -> bool:
    ...
    if not authorization:
        ...
        raise HTTPException(
            status_code=401, detail="Authentication required. Missing Authorization header."
        )
    ...
    if not token:
        ...
        raise HTTPException(status_code=401, detail="Empty token in Authorization header.")
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"✅ JWT token verified for issuer: {verified_token.get('iss', 'unknown')}"
            )
            return True

        except Exception as e:
            logger.error(f"❌ JWT verification failed: {e}")
            raise HTTPException(status_code=401, detail=f"JWT token verification failed: {str(e)}")
