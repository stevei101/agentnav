...
class ModelLoader:
    ...
    def _detect_device(self) -> str:
        ...
        try:
            ...
            if device_count == 0:
                logger.warning(
                    "⚠️  CUDA available but no devices found, falling back to CPU"
                )
                return "cpu"
    ...
    def load_model(self):
        ...
        try:
            ...
            if (
                hf_token
                and hf_token.strip()
                and hf_token.lower() not in ["dummy-token-value", "placeholder", ""]
            ):
                logger.info("   Using Hugging Face token for authentication")
                token_kwargs = {"token": hf_token}
            else:
                logger.warning(
                    "   No valid Hugging Face token found - model may require authentication"
                )
                token_kwargs = {}
