...
const statusConfig = {
  ...
  [AgentStatusValue.PROCESSING]: {
    icon: <LoaderIcon className="w-5 h-5 text-sky-400" />,
    color: 'text-sky-400',
  ...
  [AgentStatusValue.ERROR]: {
    icon: <XCircleIcon className="w-5 h-5 text-rose-400" />,
    color: 'text-rose-400',
...
export const AgentCard: React.FC<AgentCardProps> = ({ agent }) => {
  ...
  return (
    <div
      className={`flex items-center p-3 rounded-lg transition-all duration-300 ${config.bgColor}`}
    >
