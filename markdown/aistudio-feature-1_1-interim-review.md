...
| **Frontend Volumes** | `volumes: - .:/app`                     | **Functionality Break/Pollution.** This mounts the _entire repository root_ (`.`) into the frontend container's working directory, which is incorrect and includes files like the backend's code, the `.git` folder, etc.                | **Change to `volumes: - ./frontend:/app`.** This ensures only the code the frontend needs is mounted for hot-reload.                                                              |
